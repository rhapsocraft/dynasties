...
public class FetchMeal<E extends AbstractDynastyVillager> extends ExtendedBehaviour<E> {
    private static final List<Pair<MemoryModuleType<?>, MemoryStatus>> MEMORY_REQUIREMENTS;
    private CampfirePotBlockEntity targetPot = null;
    ...
    protected void start(E entity) {
        var pot = BrainUtil.getMemory(entity, ModMemoryTypes.AVAILABLE_MEAL.get());

        if (pot != null) {
            if (this.targetPot != null) {
                var targetPos = this.targetPot.getBlockPos();

                if (AIUtils.isCloseEnoughToTarget(entity, targetPos)) {
                    var extractFood = this.targetPot.getServing();
    ...
    static {
        MEMORY_REQUIREMENTS = ObjectArrayList.of(new Pair[]{
                Pair.of(ModMemoryTypes.AVAILABLE_MEAL.get(), MemoryStatus.VALUE_PRESENT),
        });
    }
}
